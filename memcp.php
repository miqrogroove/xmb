...
$avatarbox = "<select name=\"avatar\" onchange=\"document.images.avatarpic.src = this[this.selectedIndex].value;\">$avatars</select>";
eval("\$avatar = \"".template("memcp_profile_avatarlist")."\";");
closedir($dir1);
}

$setquery = $db->query("SELECT usercstatus FROM $table_settings");
$setc = $db->fetch_array($setquery);
if($setc[usercstatus] == "on") {
$customuserstatus = "<tr><td bgcolor=\"$altbg1\" width=\"22%\" class=\"tablerow\">Custom Status:</td><td bgcolor=\"$altbg2\" class=\"tablerow\"><input type=\"text\" name=\"cstatus\" size=\"30\" value=\"$member[customstatus]\"></td></tr>";
} else {
$customuserstatus = "";
}
...
$bday = str_replace(">","&gt;", $bday);
$pstatus = str_replace(">","&gt;", $pstatus);
$pstatus = str_replace("<","&lt;", $pstatus);
$pstatus = addslashes($pstatus);
$bio = addslashes($bio);
...
if($setc[usercstatus] == "on") {
$db->query("UPDATE $table_members SET customstatus='$cstatus' WHERE username='$xmbuser'");
}
