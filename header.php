...
$tables = array('attachments', 'banned', 'favorites', 'forums', 'members', 'posts', 'ranks', 'settings', 'smilies', 'templates', 'themes', 'threads', 'u2u', 'whosonline', 'words', 'buddys'); 
...
if($regviewonly == "on") {
	if($onlineuser == "xguest123" && $action != "reg" && $action != "login" && $action != "lostpw") {
		...
		$message = "$lang_reggedonly $reglink $lang_textor <a href=\"misc.php?action=login\">$lang_textlogin</a>";
		noaccess($message);
		exit;
	}
}
