...
header("Location: viewthread.php?tid=$post[tid]&page=$topicpages#pid$post[pid]"); 
exit; 
} 
} 
if($fid) { 
...
if($pid) {
 ...
 $query = $db->query("SELECT COUNT(pid) AS postnum FROM $table_posts WHERE
tid='$tid' AND pid <= '$pid'");
...
$getwoquery = $db->query("SELECT username FROM $table_whosonline WHERE
username='$post[author]' OR ip='$onlineip'");
$getwo = $db->fetch_array($getwoquery);
if($getwo && $getwo[username] == $post[author]) {
...
$onlinenow = "$lang_memberisoff";
		...
		if($post[author] != "Anonymous") {
			...
			$post[msn] = $post[msn];
			eval("\$aim = \"".template("viewthread_post_msn")."\";");
			}
