...
if($mods[$num] != "") {
$db->query("INSERT INTO $table_u2u VALUES('', '$mods[$num]', '$xmbuser', '" . time() . "', '$lang_reportsubject', '$message', 'inbox', 'yes')");
}
}
...
if($member[username] != "") {
$db->query("INSERT INTO $table_u2u VALUES('', '$member[username]', '$xmbuser', '" . time() . "', '$lang_reportsubject', '$message', 'inbox', 'yes')");
}
}
...
if($newvotecol && $xmbuser) {
	...
	$newvotecol = addslashes($newvotecol);
...
?>
<script>
function redirect() { window.location.replace("viewthread.php?tid=<?=$tid?>"); }
setTimeout("redirect();", 1250);
</script>
<?
}
