...
switch ($action) {
    ...
    case 'votepoll':
        ...
        $vote_result = $db->result($db->query("SELECT COUNT(vote_option_id) FROM $table_vote_results WHERE vote_id='$vote_id' AND vote_option_id='$postopnum'"), 0);
        if ($vote_result != 1) {
            error($lang['pollvotenotselected'], false);
        }

        // Has the user voted on this poll before?
        $voted = $db->result($db->query("SELECT COUNT(vote_id) FROM $table_vote_voters WHERE vote_id='$vote_id' AND vote_user_id='$self[uid]'"), 0);
        if ($voted === 1) {
            error($lang['alreadyvoted'], false);
        }

        // Okay, the user is about to vote
        $db->query("INSERT INTO $table_vote_voters (vote_id, vote_user_id, vote_user_ip) VALUES ('$vote_id', '$self[uid]', '".encode_ip($onlineip)."')");
        $db->query("UPDATE $table_vote_results SET vote_result=vote_result+1 WHERE vote_id='$vote_id' AND vote_option_id='$postopnum'");
