<?php
/* $Id: cinst.php,v 1.26.2.9 2004/09/24 19:09:25 Tularis Exp $ */
/*
    XMB 1.9
...
show_act("Inserting data into ".$tablepre."settings");
    $db->query("INSERT INTO ".$tablepre."settings VALUES ('English', 'Your Forums', 25, 30, 20, '7', 'on', 'on', 'on', '', 'off', 5, 45, 'off', 'on', 'off', 'off', '', 'on', 'on', 'on', 'YourDomain.com', '$full_url', 'on', 600, 'on', '$full_url', 'off', 12, 'webmaster@domain.ext', 'dd-mm-yyyy', 'on', 'off', 'on', 'on', 'on', 'off', '16', '4', 'off', 'on', 'on', 'on', 'on', 'on', 'on', '<b>Welcome to your new boards!!</b>\nModify your board to your own taste, we recommend starting with changing the settings...!', '4000', '0', '100x100', 'queries-phpsql-loadtimes-totaltime', 'no', 'off', 'on', 'off');");
show_result(X_INST_OK);
...
show_act("Creating Super Administrator Account");
    ...
    $vPassword = md5(trim($frmPassword));
    $db->query("INSERT INTO ".$tablepre."members (username, password, regdate, postnum, status, tpp, ppp) VALUES ('$vUsername', '$vPassword', ".$db->time(time()).", '0', 'Super Administrator', 30, 30);");
show_result(X_INST_OK);
