<?php
/**
* $Id: u2u.inc.php,v 1.1.2.6 2005/09/18 19:28:44 Tularis Exp $
*/
...
function u2u_send($u2uid, $msgto, $subject, $message, $u2upreview) {
    global $db, $self, $lang, $username, $SETTINGS, $table_u2u;
    ...
    if ( isset( $sendsubmit ) ) {
		...
		// floodcontrol!
		// $SETTINGS['floodctrl']
		if($db->result($db->query("SELECT count(u2uid) FROM $table_u2u WHERE msgto='$msgto' AND msgfrom='$self[username]' AND dateline > ".(time()-$SETTINGS['floodctrl'])), 0) > 0) {
...
function u2u_print($u2uid, $eMail = false) {
    global $db, $self, $table_u2u, $timeoffset, $lang, $u2uheader, $u2ufooter, $dateformat, $timecode;
