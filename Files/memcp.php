<?php
/* $Id: memcp.php,v 1.17.2.12 2004/09/24 19:10:32 Tularis Exp $ */
/*
    XMB 1.9
...
if ($action == "profile") {
    ...
    if (isset($editsubmit)) {
        ...
        $email          = isset($newemail) ? checkInput($newemail, '', '', 'javascript', false) : '';
...
elseif ($action == "favorites") {
    ...
    if (isset($favadd) && !isset($favsubmit)) {
        ...
        redirect('memcp.php?action=favorites', 0);
    }
...
elseif ($action == "subscriptions") {
    ...
    if (!isset($subadd) && !isset($subsubmit)) {
        ...
        if ($db->result($query,0) == 1) {
            ...
            redirect('memcp.php?action=subscriptions', 0);
        }
    } elseif (!isset($subadd) && isset($subsubmit)) {
