<?php
/* $Id: member.php,v 1.23.2.11 2004/09/24 19:10:32 Tularis Exp $ */
/*
    XMB 1.9
        ...
        if ($SETTINGS['notifyonreg'] != "off") {
            if ($SETTINGS['notifyonreg'] == 'u2u') {
                ...
                while ($admin = $db->fetch_array($mailquery)) {
                    $db->query("INSERT INTO $table_u2u ( u2uid, msgto, msgfrom, type, owner, folder, subject, message, dateline, readstatus, sentstatus ) VALUES ('', '$admin[username]', '$bbname', 'incoming', '$admin[username]', 'Inbox', '$lang[textnewmember]', '$lang[textnewmember2]', '" . time() . "', 'no', 'yes')");
                }
            } else {
