<?php
/* $Id: stats.php,v 1.3.2.5 2005/10/28 15:11:32 Tularis Exp $ */
/*
    XMB 1.9.2
...
// Get top 5 most viewed threads
    $viewmost = '';
    $query      = $db->query("SELECT t.views, t.tid, t.subject FROM $table_threads t, $table_forums f WHERE $restrict AND f.fid = t.fid ORDER BY views DESC LIMIT 0,5");
...
// Get top 5 most replied to threads
    $replymost = '';
    $query = $db->query("SELECT t.replies, t.tid, t.subject FROM $table_threads t, $table_forums f WHERE $restrict AND f.fid = t.fid ORDER BY replies DESC LIMIT 0,5");
...
// Get last 5 posts
    $latest = '';
    $query = $db->query("SELECT t.lastpost, t.tid, t.subject FROM $table_threads t, $table_forums f WHERE $restrict AND f.fid = t.fid ORDER BY lastpost DESC LIMIT 0,5");
...
// Get most popular forum
    $query      = $db->query("SELECT f.posts, f.threads, f.fid, f.name FROM $table_forums f WHERE $restrict AND f.fid = f.fid AND f.type='sub' OR f.type='forum' ORDER BY posts DESC LIMIT 0, 1");
