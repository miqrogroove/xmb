...
?>
										...
										<tr bgcolor="<?=$altbg2?>" class="tablerow">
											...
											<td><input type="text" name="name_<?=$i?>" value="<?=$themeinfo[name]?>" /> <a href="cp2.php?action=themes&single=<?=$themeinfo[name]?>"><?=$lang_textdetails?></a> - <a href="cp2.php?action=themes&download=<?=$themeinfo[name]?>"><?=$lang_textdownload?></a></td>
											<td><?=$members?></td>
										</tr>
...
<?
$query = $db->query("SELECT * FROM $table_ranks ORDER BY stars, posts");
...
if($action == "newsletter") {
	if(!$newslettersubmit) {
		...
		<input type="radio" value="mod" name="to"> <?=$lang_textsendmod?></td>
...
if($action == "prune") {
	if(!$_POST['prunesubmit']){
		...
		$deletetopped = "<select name=deletetopped><option value=yes SELECTED>$lang_textyes</option><option value=no>$lang_textno</option></select>";
		?>
		...
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_prune_fid?></td>
		<td align="right" bgcolor="<?=$altbg2?>"><?=$prunefid?></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_delete_topped?></td>
		<td align="right" bgcolor="<?=$altbg2?>"><?=$deletetopped?></td>
		</tr>

		</table>
		</td></tr></table>
		<center><br /><input type="submit" name="prunesubmit" value="<?=$lang_textsubmitchanges?>" /></center>
		</form>
		...
		if($fid != 0 && $fid != ''){
			$querythread = $db->query("SELECT * FROM $table_threads WHERE lastpost <= '$prunedate' AND fid='$fid' $topped");
			$prunedate = '';
			$forumprune = '';
		}else{
			if($forumprune == $lang_textall) {
				$querythread = $db->query("SELECT * FROM $table_threads WHERE lastpost <= '$prunedate' $topped");
			} else {
				$querythread = $db->query("SELECT * FROM $table_threads WHERE lastpost <= '$prunedate' AND fid='$forumprune' $topped");
			}
		}
		...
		echo "<tr bgcolor=\"$altbg2\" class=\"tablerow\"><td align=\"center\">$lang_forumpruned</td></tr>";
	}
}
...
// Patch to prevent variables to be displayed when using templates

	if(isset($editsubmit)) {
		$templatenew = addslashes($templatenew);
		$sql_check = Array('dbname', 'dbuser', 'dbpw','dbhost');
		foreach ($sql_check as $replace) {
			$find = "/\W".$replace."/si";
			$templatenew = preg_replace($find, $replace, $templatenew);
		}
...
if($action == "attachments") {
	if(!$attachsubmit && !$searchsubmit) {
		...
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwherename?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="filename" size="30" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwhereauthor?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="author" size="40" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwhereforum?></td>
		<td bgcolor="<?=$altbg2?>"><?=$forumselect?></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwheresizesmaller?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="sizeless" size="20" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwheresizegreater?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="sizemore" size="20" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwheredlcountsmaller?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="dlcountless" size="20" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwheredlcountgreater?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="dlcountmore" size="20" /></td>
		</tr>
		<tr>
		<td class="tablerow" bgcolor="<?=$altbg1?>"><?=$lang_attachmanwheredaysold?></td>
		<td bgcolor="<?=$altbg2?>"><input type="text" name="daysold" size="20" /></td>
		</tr>
		</table>
		</td></tr></table><br />
		<center><input type="submit" name="searchsubmit" value="<?=$lang_textsubmitchanges?>" /></center>
		</form>
	...
	if($searchsubmit) {
		...
		<center><input type="submit" name="deletesubmit" value="<?=$lang_textsubmitchanges?>" /></center>
