<?php
/* $Id: topicadmin.php,v 1.3.2.7 2005/10/28 15:11:32 Tularis Exp $    */
/*
    XMB 1.9.2
...
// Pre-load    templates
    loadtemplates('footer_load', 'footer_querynum', 'footer_phpsql', 'footer_totaltime','topicadmin_delete','topicadmin_openclose','topicadmin_move','topicadmin_topuntop','topicadmin_bump','topicadmin_split_row','topicadmin_split','topicadmin_merge','topicadmin_report','topicadmin_empty','topicadmin_threadprune_row','topicadmin_threadprune','topicadmin_copy','misc_feature_notavailable');
...
// Start actions...
    ...
    case 'move':
        if (!$movesubmit) {
            ...
            $queryfor = $db->query("SELECT fid, name, status FROM $table_forums WHERE fup='' AND type='forum' ORDER BY displayorder");
            ...
            while($forum = $db->fetch_array($queryfor))    {
            	...
            	$querysub = $db->query("SELECT fid, name, status FROM $table_forums WHERE fup='$forum[fid]' AND type='sub' ORDER BY displayorder");
			...
			while($group = $db->fetch_array($querygrp))    {
				...
				$queryfor = $db->query("SELECT fid, name, status FROM $table_forums WHERE fup='$group[fid]' AND type='forum' ORDER BY displayorder");
				while($forum = $db->fetch_array($queryfor))    {
					$forumselect .= "<option value=\"$forum[fid]\" ".($forum['status'] == 'off' ? 'disabled="disabled"' : '')."> &nbsp; &raquo; $forum[name]</option>";
					$querysub = $db->query("SELECT status, fid, name FROM $table_forums WHERE fup='$forum[fid]' AND type='sub' ORDER BY displayorder");
					while($sub = $db->fetch_array($querysub)) {
						$forumselect .= "<option value=\"$sub[fid]\" ".($sub['status'] == 'off' ? 'disabled="disabled"' : '').">&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &raquo; $sub[name]</option>";
					}
				}
