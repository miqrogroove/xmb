...
// Required Files - XMB (Version/Patch File) Configuration File, Database Settings File
    ...
    $versionshort       = "XMB 1.9.2";
    $versiongeneral     = 'XMB 1.9.2 Nexus';
    ...
    if ( $show_full_info) {
        ...
        $gamma              = 'Final';
        $service_pack       = '';
        $versionbuild       = 200509201330;
        $versionlong        = "Powered by XMB 1.9.2 Nexus ($alpha$beta$gamma$service_pack)".(DEBUG === true ? ' (Debug)' : '');
    } else {
        $alpha              = '';
        $beta               = '';
        $gamma              = '';
...
//	while others are available via IE and/or opera
	...
	if(false !== strpos($_SERVER['HTTP_USER_AGENT'], 'Gecko')) {
...
// Get themes, [fid, [tid]]
    if (isset($tid)) {
        ...
        $forumtheme = $db->result($db->query("SELECT theme FROM $table_forums WHERE fid='$fid'"), 0);
    }
...
// Make theme-vars semi-global
    ...
    if (false !== strpos($boardimg, ",")) {
        ...
        $logo = '<object type="application/x-shockwave-flash" data="'.$flashlogo[0].'" width="'.$flashlogo[1].'" height="'.$flashlogo[2].'"><param name="movie" value="'.$flashlogo[0].'" /></object>';
    } else {
        $l = array();
