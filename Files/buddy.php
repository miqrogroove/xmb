<?php
/* $Id: buddy.php,v 1.11.2.5 2004/09/24 19:10:30 Tularis Exp $ */
/*
    XMB 1.9
...
// Pre-load templates
loadtemplates('buddylist_edit_buddy','buddylist_edit','buddylist_buddy_online','buddylist_buddy_offline','buddylist','css', 'buddy_u2u_inv', 'buddy_u2u_on', 'buddy_u2u_off', 'buddy_u2u');
    ...
    foreach($buddys as $key=>$buddy) {
        if (empty($buddy) || (strlen(trim($buddy)) == 0) ) {
            ...
            if ($db->result($q, 0) > 0) {
                ...
                // not yet on the list.
                $q = $db->query("SELECT count(*) FROM $table_members WHERE username='$buddy'");
                if($db->result($q, 0) < 1) {
