...
// Check for login status
	if(!$xmbuser || !$xmbpw) {
		$lang_u2unotloggedin = stripslashes($lang_u2unotloggedin);
...
// Define necessary functions
	function blistmsg($message, $redirect="") {
		...
		echo stripslashes($blistmessage);
	}
	...
	switch($action){
		case 'add':
			if($buddy == "") {
				...
				$query = $db->query("SELECT * FROM $table_buddys WHERE username='$xmbuser' AND buddyname='$buddy'");
		...
		case 'edit':
			if(!$editsubmit) {
				...
				echo stripslashes($edit);
			}else{
				if(trim($newbuddy1) != '') {
					$query = $db->query("SELECT * FROM $table_buddys WHERE username='$xmbuser' AND buddyname='$newbuddy1'");
					if($db->fetch_array($query)) {
				...
				if($newbuddy2 != "") {
					$query = $db->query("SELECT * FROM $table_buddys WHERE username='$xmbuser' AND buddyname='$newbuddy2'");
					if($db->fetch_array($query)) {
