<?php
/* $Id: functions.php,v 1.3.2.57 2007/03/20 03:28:00 ajv Exp $ */
/*
...
function error($msg, $showheader=true, $prepend='', $append='', $redirect=false, $die=true, $return_as_string=false, $showfooter=true) {
    ...
    global $bgcode, $charset, $threadSubject;
...
function dump_query($resource, $header=true) {
    ...
    if (!$db->error()) {
        ...
        if ( $header) {
            ?><tr class="category" background-color: <?php echo $THEME['altbg2']?>" align="center"><?php
            for($i=0;$i<$count;$i++) {
                echo '<td align="left">';
        ...
        while($a = $db->fetch_array($resource, SQL_NUM)) {
            ?><tr style="background-color: <?php echo $THEME['altbg1']?>" class="ctrtablerow"><?php
            for($i=0;$i<$count;$i++) {
                echo '<td align="left">';
