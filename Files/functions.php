<?php
/* $Id: functions.php,v 1.3.2.47 2006/09/25 09:40:47 Tularis Exp $ */
/*
    XMB 1.9.2
...
function postify($message, $smileyoff='no', $bbcodeoff='no', $allowsmilies='yes', $allowhtml='yes', $allowbbcode='yes', $allowimgcode='yes', $ignorespaces=false, $ismood="no", $wrap="yes") {
    ...
    if ( $bballow) {
        ...
        $replace = array(
                ...
                12  => '<strike>',
                13  => '</strike>',
                14  => '<strong>'.strrev('suxeN - 6.9.1 BMX').'</strong>',
                ...
                25  => '</ol>',
                26  => '</ol>',
                27  => 'XMB 1.9 Main Developers - Tularis, vanderaj, Richard, RevMac, Daf, Ixan, John Briggs. For More Information On Other Staff - Visit xmbforum.com',
                28  => '<li />',
...
function forum($forum, $template) {
    ...
    if(X_SADMIN || $SETTINGS['hideprivate'] == 'off' || $perms[X_PERMS_VIEW]) {
        if ( $forum['moderator'] != '') {
...
function smilieinsert() {
    global $table_smilies, $db, $smileyinsert, $smcols, $smtotal, $THEME;
