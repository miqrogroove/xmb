<?php
/* $Id: post.php,v 1.46.2.13 2004/09/24 19:10:32 Tularis Exp $ */
/*
    XMB 1.9
...
$listed_icons = 0;
$icons = '<input type="radio" name="posticon" value="" /> &nbsp;';
...
$allowhtml      = ( isset($forums['allowhtml']) && $forums['allowhtml'] == "yes" )          ? $lang['texton'] : $lang['textoff'];
...
if( !empty($posticon) ) {
    ...
    $icons = str_replace('<input type="radio" name="posticon" value="'.$posticon.'" />', '<input type="radio" name="posticon" value="'.$posticon.'" checked="true" />', $icons);
}else{
    $thread['icon'] = '';
    $icons = str_replace('<input type="radio" name="posticon" value="" />', '<input type="radio" name="posticon" value="" checked="true" />', $icons);
}
...
if(isset($previewpost)) {
    ...
    $subject = checkInput(stripslashes($subject), '', '', '', false);
    $message = checkInput(stripslashes($message), '', '', '', true);
...
if($action == "newthread") {
    ...
    if(!isset($topicsubmit) || !$topicsubmit) {
        ...
        $db->query("INSERT INTO $table_threads ( tid, fid, subject, icon, lastpost, views, replies, author, closed, topped, pollopts ) VALUES ('', '$fid', '$subject', '$posticon', '$thatime|$username', '0', '0', '$username', '', '', '$pollanswers')");
        $tid = $db->insert_id();

        $message = checkOutput($message, 'on');
        $message = checkInput($message, '', '', '', true);

        $db->query("INSERT INTO $table_posts ( fid, tid, pid, author, message, subject, dateline, icon, usesig, useip, bbcodeoff, smileyoff ) VALUES ('$fid', '$tid', '', '$username', '$message', '$subject', ".$db->time($thatime).", '$posticon', '$usesig', '$onlineip', '$bbcodeoff', '$smileyoff')");
        $pid = $db->insert_id();
    ...
    if(!isset($replysubmit) || !$replysubmit) {
        ...
        if($closed == "yes" && !X_STAFF) {
            ...
            $db->query("INSERT INTO $table_posts ( fid, tid, pid, author, message, subject, dateline, icon, usesig, useip, bbcodeoff, smileyoff ) VALUES ('$fid', '$tid', '', '$username', '$message', '$subject', ".$db->time(time()).", '$posticon', '$usesig', '$onlineip', '$bbcodeoff', '$smileyoff')");
            $pid = $db->insert_id();
            ...
            $subquery = $db->query("SELECT m.email, m.lastvisit, m.ppp FROM $table_favorites f LEFT JOIN $table_members m ON (m.username=f.username) WHERE f.type='subscription' AND f.tid='$tid' AND f.username != '$username'");
            while($subs = $db->fetch_array($subquery)) {
                ...
                $topicpages = quickpage($postsnum, $subs['ppp']);
    ...
    if(!isset($editsubmit)) {
        ...
        /*
        if($self['status'] != 'Super Administrator' && $forums['password'] != '') {
        ...
        */
        ...
        if((X_STAFF && $status1 == 'Moderator') || $username == $orig['author']) {
            ...
            if(($file = get_attached_file($_FILES['attach'], $forums['attachstatus'], $max_attach_size)) !== false) {
                $db->query("INSERT INTO $table_attachments ( aid, tid, pid, filename, filetype, filesize, attachment, downloads ) VALUES ('', '$tid', '$pid', '$filename', '$attach[type]', '$filesize', '$file', '0')");
            }
