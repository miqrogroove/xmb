...
if(!$xmbuser || !$xmbpw) {
	if($coppa == "on") {
		$reglink = "<a href=\"member.php?action=coppa\">$lang_regques</a>";
	} else {
		$reglink = "<a href=\"member.php?action=reg\">$lang_regques</a>";
	}
	...
	if($topicsubmit) {
		...
		if($attach != "none" && $attach != "" && $forums[attachstatus] != "off") {
			...
			if($attach_type=="application/x-gzip-compressed" || $attach_type=="application/zip"){
				$attachedfile = base64_encode($attachedfile);
			}
...
if($action == "reply") {
	$qry = $db->query("SELECT closed FROM $table_threads WHERE tid = '$tid' AND fid = '$fid'");
