...
$tables = array('attachments', 'banned', 'favorites', 'forums', 'members', 'posts', 'ranks', 'settings', 'smilies', 'templates', 'themes', 'threads', 'u2u', 'whosonline', 'words');
...
if($action == "1_50RC4") {
	...
	$db->query("DROP TABLE IF EXISTS $table_attachments");
	...
	echo "Done<br>Moving attachments..";
	$query = $db->query("SELECT * FROM $table_posts WHERE attach != ''");
	while($attachment = $db->fetch_array($query)) {
		$attachmentexploded = explode("|^!*!^|", $attachment[attach]);
		$attachsize = strlen($attachmentexploded[2]);
		$attachmentexploded[2] = addslashes($attachmentexploded[2]);
		$db->query("INSERT INTO $table_attachments VALUES ('', '$attachment[tid]', '$attachment[pid]', '$attachmentexploded[0]', '$attachmentexploded[1]', '$attachsize', '$attachmentexploded[2]', '0')");
	}
	echo "Done<br>Dropping attach in $table_posts..";
	$db->query("ALTER TABLE $table_posts DROP attach");
	echo "Done";
	echo "<br>Updating settings...";
	$db->query("ALTER TABLE $table_settings ADD usercstatus CHAR(3) NOT NULL default ''");
	echo "Done";
}
if($action == "1_50") {
	if($convpasses != "yes") {
	...
	usercstatus char(3) NOT NULL default '',
	);");
	...
	$db->query("INSERT INTO $table_settings VALUES('$langfile', '$bbname', '$postperpage', '$topicperpage', '$hottopic', '$theme', '$bbstatus', '$whosonlinestatus', '$regstatus', '$bboffreason', '$regviewonly', '$floodctrl', '$memberperpage', '$catsonly', '$hideprivate', '$emailcheck', '$bbrules', '$bbrulestxt', '$searchstatus', '$faqstatus', '$memliststatus', '$sitename', '$siteurl', '$avastatus', '$u2uquota', '$gzipcompress', '$boardurl', '$coppa', '$timeformat', '$adminemail', '$dateformat', '$sigbbcode', '$sightml', '$reportpost', 'on', 'on', 'on', '16', '4', 'off')");
		echo "Done<br>";
