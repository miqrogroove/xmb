...
if($action == "view") {
	...
	if(($u2u[msgfrom] == $uid) || ($u2u[msgto] == $uid || $status=="Super Administrator")) {
		...
		if($u2u[msgfrom] != $uid) {
			eval("\$refwdlinks = \"".template("u2u_view_refwdlinks_admin")."\";");
		}
		eval("\$view = \"".template("u2u_view_admin")."\";");
		$u2uview = stripslashes($u2uview);
		echo $view;
	}
}
