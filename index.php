...
if(!$gid) {
	...
	if($xmbuser) {
		...
		if($coppa == "on") {
			$reglink = "<a href=\"member.php?action=coppa\">$lang_texthere</a>";
		} else {
			$reglink = "<a href=\"member.php?action=reg\">$lang_texthere</a>";
		}
	...
	if($whosonlinestatus == "on") {
		...
		for($mnum=0; $mnum<$membercount; $mnum++) {
			...
			elseif($online[status] == "Moderator" || $online[status] == "Super Moderator") {
				$pre = "<b>";
				$suf = "</b>";
			}
			else {
