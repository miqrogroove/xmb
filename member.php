<?php
/* $Id: member.php,v 1.3.2.50 2007/05/22 21:07:44 ajv Exp $ */
/*
...
loadtemplates('member_coppa','member_reg_rules','member_reg_password','member_reg_avatarurl','member_reg_avatarlist','member_reg','member_profile_email','member_profile','misc_feature_not_while_loggedin','misc_feature_notavailable');
smcwcache();
...
$tpp           = isset($tpp) ? (int) $tpp : $SETTINGS['topicperpage'];
$ppp           = isset($ppp) ? (int) $ppp : $SETTINGS['postperpage'];
...
if ($action == "coppa") {
    ...
    if($SETTINGS['pruneusers'] > 0) {
        $prunebefore = time()-(60 * 60 * 24 * $SETTINGS['pruneusers']);
    ...
    if (!isset($regsubmit)) {
        eval('echo "'.template('header').'";');
        if ($SETTINGS['bbrules'] == "on" && !isset($rulesubmit)) {
            $bbrulestxt = nl2br(stripslashes(stripslashes($SETTINGS['bbrulestxt'])));
            ...
            if($SETTINGS['timeformat'] == 24) {
                $timeFormat12Checked = '';
                $timeFormat24Checked = 'checked="checked"';
            } else {
                $timeFormat12Checked = 'checked="checked"';
                $timeFormat24Checked = '';
            }
        ...
        $timeformatnew = isset($timeformatnew) ? checkInput($timeformatnew, '', '', 'script', false) : $SETTINGS['timeformat'];
        ...
        $db->query("INSERT INTO $table_members (username, password, regdate, postnum, email, site, aim, status, location, bio, sig, showemail, timeoffset, icq, avatar, yahoo, customstatus, theme, bday, langfile, tpp, ppp, newsletter, regip, timeformat, msn, ban, dateformat, ignoreu2u, lastvisit, mood, pwdate, invisible, u2ufolders, saveogu2u, emailonu2u, useoldu2u, webcam) VALUES ('$username', '$password', ".$db->time(time()).", 0, '$email', '$site', '$aim', '$self[status]',  '$locationnew', '$bio', '$sig', '$showemail', '$timeoffset1', '$icq', '$avatar', '$yahoo', '', '$thememem', '$bday', '$langfilenew', $tpp, $ppp,  '$newsletter', '$onlineip', '$timeformatnew', '$msn', '', '$dateformatnew', '', '', '$mood', '', 0, '', '$saveogu2u', '$emailonu2u', 'no', '$webcam')");
        ...
        if ($SETTINGS['notifyonreg'] != "off") {
            if ($SETTINGS['notifyonreg'] == 'u2u') {
                ...
                while ($admin = $db->fetch_array($mailquery)) {
                    $db->query("INSERT INTO $table_u2u (msgto, msgfrom, type, owner, folder, subject, message, dateline, readstatus, sentstatus) VALUES ('$admin[username]', '".addslashes($SETTINGS['bbname'])."', 'incoming', '$admin[username]', 'Inbox', '$lang[textnewmember]', '$lang[textnewmember2]', ".time().", 'no', 'yes')");
                }
            } else {
        ...
        if ($SETTINGS['emailcheck'] == "on") {
            altMail($email, $lang['textyourpw'], $lang['textyourpwis']." \n\n$username\n$password2", "From: ".$SETTINGS['bbname']." <".$SETTINGS['adminemail'].">");
        } else {
        ...
        eval('echo "'.template('header').'";');
        echo ($SETTINGS['emailcheck'] == "on") ? '<center><span class="mediumtxt">'.$lang['emailregpw'].'</span></center>' : '<center><span class="mediumtxt">'.$lang['regged'].'</span></center>';
    ...
    if (!$member) {
        ...
        if ($memberinfo['uid'] == '') {
            ...
            $memberinfo['regdate'] = printGmDate($memberinfo['regdate']);
            ...
            $stars = str_repeat('<img src="'.$THEME['imgdir'].'/star.gif" alt="*" />', $rank['stars']);
            ...
            if (!($memberinfo['lastvisit'] > 0)) {
                ...
                $lastvisitdate = printGmDate($memberinfo['lastvisit']);
                $lastvisittime = printGmTime($memberinfo['lastvisit']);
            ...
            if($memberinfo['bday'] === iso8601_date(0,0,0)) {
                ...
                $memberinfo['bday'] = printGmDate(MakeTime(12,0,0,substr($memberinfo['bday'],5,2),substr($memberinfo['bday'],8,2),substr($memberinfo['bday'],0,4)), null, 0);
            }
            ...
            while($post = $db->fetch_array($pq)) {
                ...
                $lastpostdate = printGmDate($post['dateline']);
                $lastposttime = printGmTime($post['dateline']);
