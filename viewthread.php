...
if($pid) { 
$pquery = $db->query("SELECT COUNT(*) FROM $table_posts WHERE tid = '$tid'"); 
$npages = $db->result($pquery, 0); 
if($npages%$ppp == 0) { 
$page = $result[posts]/$ppp; 
} else { 
$page = ceil($npages/$ppp); 
} 
}
...
$thread[pollopts] = stripslashes($thread[pollopts]); 
