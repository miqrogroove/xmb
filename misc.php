...
$sql = "SELECT p.*, t.tid AS ttid, t.subject AS tsubject FROM $table_posts p, $table_threads t";
...
if($srchtxt) {
$sql .= " WHERE (p.message LIKE '%$srchtxt%' OR p.subject LIKE '%$srchtxt%' OR t.subject LIKE '%$srchtxt') AND p.dateline >= '$srchfrom'";
}
elseif($srchtxt == "" && $srchuname == "" && $srchfid != "all" || $srchfid == "") {
$sql .= " WHERE p.fid='$srchfid' AND p.dateline >= '$srchfrom'";
}
elseif($srchtxt == "" && $srchuname != "") {
if($srchfid == "all" || $srchfid != "") {
$sql .= " WHERE p.author='$srchuname' AND p.dateline >= '$srchfrom'";
}
}

if($srchfid != "all" && $srchtxt != "" && $srchuname == "") {
$sql .= " AND p.fid='$srchfid' AND p.dateline >= '$srchfrom'";
}
...
$sql .= " AND p.author='$srchuname' AND p.dateline >= '$srchfrom'";
}
if($srchtxt) {
$sql .= " AND p.tid=t.tid";
}
...
if($lostpwsubmit) {
$query = $db->query("SELECT username, email FROM $table_members WHERE username='$username' OR email='$email'");
$member = $db->fetch_array($query);
...
$db->query("UPDATE xmb_members SET password='$newmd5pass' WHERE username='$member[username]' OR email='$member[email]'");

mail("$member[email]", "$lang_textyourpw", "$lang_textyourpwis\n\n$member[username]\n$newpass", "$lang_textfrom $adminemail");
