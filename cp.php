...
<tr bgcolor="<?=$altbg2?>" class="tablerow"> 
<td><?=$lang[dupemailcheck]?></td> 
<td><select name="dupemailnew"><option value="on" <?=$dupemailon?>><?=$lang[texton]?></option><option value="off" <?=$dupemailoff?>><?=$lang[textoff]?></option></select></td> 
</tr>
...
mysql_query("UPDATE $table_members SET status='$status', customstatus='$cusstatus', password='$pw' WHERE uid='$mem[uid]'") or die(mysql_error());
}
}
}
