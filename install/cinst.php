<?php
/* $Id: cinst.php,v 1.1.2.35 2007/04/25 08:55:51 Tularis Exp $ */
/*
...
show_act("Creating Super Administrator Account");
     $db->query("INSERT INTO ".$tablepre."members (`username`, `password`, `regdate`, `email`, `status`, `bio`, `sig`, `showemail`, `theme`, `langfile`, `timeformat`, `dateformat`, `ignoreu2u`, `mood`, `pwdate`, `u2ufolders`, `tpp`, `ppp`, `saveogu2u`, `emailonu2u`, `useoldu2u`) VALUES ('$vUsername', '$vPassword', $myDate, '$vEmail', 'Super Administrator', '', '', 'no', '0', 'English', 24, 'dd-mm-yyyy', '', '', $myDate, '', 30, 30, 'yes', 'no', 'no');");
show_result(X_INST_OK);

// Try to remove all files now
show_act('Removing XMB 1.8 log file (if installed)');
if (file_exists(ROOT . 'cplogfile.php') && !@unlink(ROOT . 'cplogfile.php')) {
    show_result(X_INST_SKIP);
    error('Permission Error', 'The old logfile ("cplogfile.php") has been found on the server, but could not be removed. Please remove it as soon as possible.', false);
} else {
    show_result(X_INST_OK);
}

show_act('Removing "Fix Hack" tool (if installed)');
if (file_exists(ROOT . 'fixhack.php') && !@unlink(ROOT . 'fixhack.php')) {
    show_result(X_INST_SKIP);
    error('Permission Error', 'The hack repair tool ("fixhack.php") has been found on the server, but could not be removed. Please remove it as soon as possible.', false);
} else {
    show_result(X_INST_OK);
}

show_act('Removing u2uadmin spy tool (if installed)');
...
show_act('Removing repair tool (if installed)');
if (file_exists(ROOT . 'repair.php') && !@unlink(ROOT . 'repair.php')) {
    show_result(X_INST_SKIP);
    error('Permission Error', 'The repair forum tool ("repair.php") has been found on the server, but could not be removed. Please remove it as soon as possible.', false);
} else {
    show_result(X_INST_OK);
}
...
show_act('Removing Upgrade files (if installed)');
if ( file_exists('./Upgrade') ) {
    rmFromDir('Upgrade');
    if ( file_exists('./Upgrade') ) {
        show_result(X_INST_SKIP);
        error('Permission Error', 'The upgrade utility has been found on the server, but could not be removed. Please remove it as soon as possible.', false);
    }
} else {
    show_result(X_INST_OK);
}

show_act('Removing install directory');
rmFromDir('install');
clearstatcache();
