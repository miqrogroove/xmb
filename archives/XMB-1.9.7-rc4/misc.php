<?php
/* $Id: misc.php,v 1.3.2.36 2007/04/02 22:44:37 ajv Exp $ */
/*
        ...
        case 'login':
            if (!isset($loginsubmit)) {
                ...
                if ($query && $db->num_rows($query) == 1) {
                    ...
                    /* 
                     * OLD CODE WARNING. This is being taken out as it seems to be a complete WAFTAM
                     */
...
//                    }
        ...
        case 'logout':
            ...
            put_cookie("xmbuser", '', time() - 3600, $cookiepath, $cookiedomain);
            put_cookie("xmbpw", '', time() - 3600, $cookiepath, $cookiedomain);
            
            // loop trough all password-forum-cookies and remove them
            foreach($_COOKIE as $key=>$val) {
                if (preg_match('#^forumPW\[([0-9]+)\]$#', $key)) {
                    put_cookie($key, '', time() - 3600, $cookiepath, $cookiedomain);
                }
            }
        ...
        case 'search':
            ...
            if (!isset($searchsubmit) && !isset($page)) {
                ...
                if ($searchsubmit || $page ) {
                    ...
                    $sql .=" GROUP BY dateline ORDER BY dateline DESC LIMIT $start, ".$self['ppp'];
        ...
        case 'lostpw':
            if (!$lostpwsubmit) {
                ...
                altMail($member['email'], '['.$SETTINGS['bbname'].'] '.$lang['textyourpw'], $lang['textyourpwis']."\n\n".$member['username']."\n".$newpass, "From: ".$SETTINGS['bbname']." <".$SETTINGS['adminemail'].">");
        ...
        case 'list':
            ...
            if (isset($where) && isset($where[0]) && $where[0] != '') {
                $q = implode(' AND', $where);
                $querymem = $db->query("SELECT * FROM $table_members WHERE $q ORDER BY $orderby $desc LIMIT $start_limit, ".$SETTINGS['memberperpage']);
                $num = $db->result($db->query("SELECT count(uid) FROM $table_members WHERE $q"), 0);
            } else {
                $querymem = $db->query("SELECT * FROM $table_members ORDER BY $orderby $desc LIMIT $start_limit, ".$SETTINGS['memberperpage']);
            ...
            if (!$SETTINGS['memberperpage']) { // unneeded ?
        ...
        case 'smilies':
            ...
            $SETTINGS['smtotal'] = 0;   // makes sure that the total # of smilies isn't limited here!
