<?php
/* $Id: cp2.php,v 1.3.2.31 2007/05/25 21:27:33 FunForum Exp $ */
/*
...
if ($action == "restrictions") {
    if (!isset($restrictedsubmit)) {
        ...
        <tr class="category">
        <td colspan="4"><span class="smalltxt"><strong><font style="color: <?php echo $THEME['cattext']?>"><?php echo $lang['textnewcode']?></font></strong></span></td>
        </tr>
        <tr class="tablerow">
        ...
        while ($restricted = $db->fetch_array($queryrestricted)) {
            ...
            $delete = (isset(${'delete'.$restricted['id']})  ? $restricted['id'] : 0);
            $case   = (isset(${'case'.$restricted['id']})    ? 1 : 0);
            $partial= (isset(${'partial'.$restricted['id']}) ? 1 : 0);

            if ( $delete == $restricted['id']) {
        ...
        if ($newname != "") {
            $newcase   = (isset($newcase)    ? 1 : 0);
            $newpartial= (isset($newpartial) ? 1 : 0);
...
elseif ($action == "themes") {
    if (!isset($themesubmit) && !isset($single) && !isset($importsubmit)) {
        ...
        <tr>
        <td class="altbg2 tablerow" align="center" colspan="3"><input type="submit" class="submit" value="<?php echo $lang['textsubmitchanges']?>" /><input type="hidden" name="orig" value="<?php echo $single?>" /></td>
        </tr>
        </table>
        </td>
        </tr>
        ...
        <tr>
        <td class="altbg2 tablerow" align="center" colspan="2"><input class="submit" type="submit" value="<?php echo $lang['textsubmitchanges']?>" /><input type="hidden" name="newtheme" value="<?php echo $single?>" /></td>
        </tr>
        </table>
        </td>
        </tr>
...
elseif ($action == "smilies") {
    if (!isset($smiliesubmit)) {
        ...
        echo '<tr class="tablerow altbg2"><td align="center">'.$lang['smilieupdate'].'</td></tr>';
    }
}
...
elseif ($action == "censor") {
    ...
    if (isset($censorsubmit)) {
        ...
        while ($censor = $db->fetch_array($querycensor)) {
            ...
            $delete  = (isset(${'delete'.$censor['id']}) ? $censor['id'] : 0);
...
elseif ($action == "templates") {
    ...
    if (isset($restoresubmit)) {
        ...
        echo '<tr class="tablerow altbg2"><td align="center">'.$lang['templatesrestoredone'].'</td></tr>';
    }
    ...
    if (isset($deletesubmit)) {
        ...
        echo '<tr class="tablerow altbg2"><td align="center">'.$lang['templatesdelete'].'</td></tr>';
    }
...
elseif ($action == "attachments") {
    ...
    if (isset($searchsubmit)) {
        ...
        $query = $db->query("SELECT a.*, p.*, t.tid, t.subject AS tsubject, f.name AS fname FROM $table_attachments a, $table_posts p, $table_threads t, $table_forums f WHERE a.pid=p.pid AND t.tid=a.tid AND f.fid=p.fid $restriction $orderby");
            ...
            ?>
            <tr>
            <td class="altbg1 tablerow" align="center" valign="middle"><a href="cp2.php?action=delete_attachment&amp;aid=<?php echo $attachment['aid']?>">Delete</a>
    ...
    if (isset($deletesubmit)) {
        ...
        echo '<tr class="tablerow altbg2"><td align="center">'.$lang['textattachmentsupdate'].'</td></tr>';
    }
}
...
elseif ($action == 'restore_attachments') {
    ...
    if (!is_readable('./attachments')) {
        ...
        echo '<tr class="tablerow altbg2"><td align="center">'.$i.' '.$lang['attachments_num_restored'].'</td></tr>';
    }
}
