<?php
/* $Id: functions.php,v 1.3.2.61 2007/06/14 00:00:45 ajv Exp $ */
/*
...
function template($name) {
    ...
    if ( $name != 'phpinclude' && $comment_output === true && defined('DEBUG') && DEBUG ) {
...
function postify($message, $smileyoff='no', $bbcodeoff='no', $allowsmilies='yes', $allowhtml='yes', $allowbbcode='yes', $allowimgcode='yes', $ignorespaces=false, $ismood="no", $wrap="yes") {
    global $bordercolor, $table_words, $table_forums, $table_smilies, $db, $smiliecache, $censorcache, $smiliesnum, $wordsnum, $versionbuild, $lang, $fontsize, $THEME;
...
function forum($forum, $template) {
    global $SETTINGS, $THEME, $lang, $self;
    global $oldtopics, $lastvisit, $lastvisit2;
...
function smilieinsert() {
    global $table_smilies, $db, $SETTINGS, $THEME;
    ...
    if($SETTINGS['smileyinsert'] == 'on' && $SETTINGS['smcols'] != '') {
        if($SETTINGS['smtotal'] == 0) {
            $querysmilie = $db->query("SELECT * FROM $table_smilies WHERE type='smiley' ORDER BY code DESC");
        } else {
            $querysmilie = $db->query("SELECT * FROM $table_smilies WHERE type='smiley' ORDER BY code DESC LIMIT 0, ".$SETTINGS['smtotal']);
        }
        ...
        if(($smilienum = $db->num_rows($querysmilie)) > 0){
            ...
            for($i=0;$i<count($sms);$i++) {
                $smilies .= $sms[$i];
                if(($i+1)%$SETTINGS['smcols'] == 0) {
                    // we have reached the Xth smiley, end row
                    $smilies .= '</tr>';
                    if(($i+1) < $SETTINGS['smtotal']) {
                        // check if we have reached the end already
                        $smilies .= '<tr>';
                    }
                }
            }
            if($smilienum%$SETTINGS['smcols'] > 0) {
                // not all cols will be filled entirely.
                $left = $SETTINGS['smcols']-($smilienum%$SETTINGS['smcols']);
                for($i=0;$i<$left;$i++) {
                    $smilies .= '<td>&nbsp;</td>';
                }
                $smilies .= '</tr>';
...
*/
function end_time() {
    global $footerstuff, $footerads, $starttime, $SETTINGS, $db, $lang;
...
function error($msg, $showheader=true, $prepend='', $append='', $redirect=false, $die=true, $return_as_string=false, $showfooter=true) {
    global $footerstuff, $lang, $THEME, $SETTINGS, $versionlong, $copyright, $footerads, $catcss, $cssInclude;
    global $versionbuild, $versioncompany, $bbcodescript, $topbgcode, $lastvisittext, $newu2umsg, $notify;
    global $links, $pluglink, $logo, $navigation;
    global $bgcode, $charset, $btitle, $debugheader;
...
* @param    $showheader     boolean, display the header if true (default)
* @param    $prepend        string, html to prepend to the message body
* @param    $append         string, html to append to the message body
* @param    $redirect       string = url to redirect, false = no redirect
* @param    $die            boolean, stop if true (default), otherwise continue
* @param    $return_as_string see return value
* @param    $showfooter     boolean, display the footer if true (default)
* @return   string          if return as string is true, will contain the error message
*/
...
function dump_query($resource, $header=true) {
    ...
    if (!$db->error()) {
        ...
        if ( $header) {
            ?><tr class="category altbg2" align="center"><?php
            for($i=0;$i<$count;$i++) {
                echo '<td align="left">';
                echo '<span style="font-weight: bold; color: '.$THEME['cattext'].'">'.$db->field_name($resource, $i).'</span>';
                echo '</td>';
            }
            echo '</tr>';
        }
        ...
        while($a = $db->fetch_array($resource, SQL_NUM)) {
            ?><tr class="altbg1 ctrtablerow"><?php
            for($i=0;$i<$count;$i++) {
                echo '<td align="left">';
...
function validatePpp() {
    ...
    if($self['ppp'] == 0) {
        ...
        $self['ppp'] = is_numeric($self['ppp']) ? (int) $self['ppp'] : $SETTINGS['postperpage'];
    }
...
function validateTpp() {
    ...
    if($self['tpp'] == 0) {
        ...
        $self['tpp'] = is_numeric($self['tpp']) ? (int) $self['tpp'] : $SETTINGS['topicperpage'];
    }
...
function logAction($action, $data, $type=X_LOG_ADMIN) {
    global $self, $db;
    global $table_logs, $onlineip;
    ...
    $data['ip']     = $onlineip;
    $log['data']    = base64_encode(serialize($data));
    //$db->query("INSERT INTO $table_logs (`uid`, `type`, `action`, `data`, `dateline`) VALUES ($log[uid], '$log[type]', '$log[action]', '".print_r(debug_backtrace(), true)."', $log[dateline])");
    
    $db->query("INSERT INTO $table_logs (`uid`, `type`, `action`, `data`, `dateline`) VALUES ($log[uid], '$log[type]', '$log[action]', '$log[data]', $log[dateline])");
}
...
/**
* MakeTime() - new mktime() function
*
* Replacement for mktime that does dates before 1970
*
* Function found at http://www.php.net/mktime
*
...
function handlePasswordDialog($fid, $file, $args=false) {
    global $SETTINGS, $THEME, $self, $lang;
    ...
    if(isset($_POST['pw'])) {
        $pass = $db->result($db->query("SELECT password FROM $table_forums WHERE fid=$fid"), 0);
...
function bbcodeinsert() {
    global $THEME, $SETTINGS, $lang, $spelling_lang;
...
// Here lie functions from UltimaBB 1.0 (supports U2U)

/**
* Board title
*
* Use this to set the board's title in the HTML title element
*
* @param   string   $btadd   false == initialize the board title, otherwise add the string to the list
* @param   boolean   $btdash   add a dash before the element, use when you want the btadd on the RHS of the dash
*/
