<?php
/* $Id: upgrade.lib.php,v 1.4 2006/09/19 00:17:18 Tularis Exp $ */
/*
    ...
    function fixForumPerms($v) {
        ...
        ***/        
        switch($v) {
            case 0:
                // store
                $q = $this->db->query("SELECT fid, private, userlist, postperm, guestposting, pollstatus FROM ".$this->tablepre."forums");
    ...
    function fixBirthdays($v) {
        ...
        switch($v) {
            case 0:
                // store
                $cache = array();
                $cachedLaguages = array();
                
                $q = $this->db->query("SELECT uid,birthday,langfile FROM ".$this->tablepre."members");
                while($m = $this->db->fetch_array($q)) {
                    if(strlen($m['birthday']) == 0) {
                        continue;
                    }
                    
                    // check if the birthday isn't in proper format anyway
                    $p = explode('-', $m['birthday']);
                    ...
                    if(!isset($cachedLanguages[$m['langfile']])) {
                        ...
                        $lang = $cachedLanguages[$m['langfile']];
                    }
            ...
            case 1:
                // restore
                if(count($cache) > 0) {
                    $this->db->query("UPDATE ".$this->tablepre."members SET birthday='0000-00-00'");
                    foreach($cache as $uid=>$bd) {
                        $this->db->query("UPDATE ".$this->tablepre."members SET birthday='$bd' WHERE uid=$uid");
                    }
                }
                break;
        }
