<?php
/* $Id: tools.php,v 1.3.2.12 2007/05/22 21:07:45 ajv Exp $ */
/*
...
switch ($action) {
    case 'fixftotals':
        ...
        nav($lang['tools']);
        echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_forumtotal'].'</td></tr></table></td></tr></table>';
        end_time();
        eval('echo "'.template('footer').'";');
        exit;
        break;
    ...
    case 'fixttotals':
        ...
        nav($lang['tools']);
        echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_threadtotal'].'</td></tr></table></td></tr></table>';
        end_time();
        eval('echo "'.template('footer').'";');
        exit;
        break;
    ...
    case 'fixmposts':
        ...
        nav($lang['tools']);
        echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_mempost'].'</td></tr></table></td></tr></table>';
        end_time();
        eval('echo "'.template('footer').'";');
        exit;
        break;
    ...
    case 'fixlastposts':
        ...
        nav($lang['tools']);
        echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_lastpost'].'</td></tr></table></td></tr></table>';
        end_time();
        eval('echo "'.template('footer').'";');
        exit;
        break;
    ...
    case 'updatemoods':
        ...
        nav($lang['tools']);
        echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_mood'].'</td></tr></table></td></tr></table>';
        end_time();
        eval('echo "'.template('footer').'";');
        exit;
        break;
    ...
    case 'u2udump':
        if (!isset($_POST['yessubmit'])) {
            echo '<tr class="ctrtablerow altbg2"><td>'.$lang['u2udump_confirm'].'<br /><form action="tools.php?action=u2udump" method="post"><input type="submit" name="yessubmit" value="'.$lang['textyes'].'" /> - <input type="submit" name="yessubmit" value="'.$lang['textno'].'" /></form></td></tr>';
        } elseif ($lang['textyes'] == $yessubmit) {
            $db->query("DELETE FROM $table_u2u");
            
            nav($lang['tools']);
            echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_u2u'].'</td></tr></table></td></tr></table>';
            end_time();
            eval('echo "'.template('footer').'";');
            exit;
        } else {
    ...
    case 'whosonlinedump':
        if (!isset($_POST['yessubmit'])) {
            echo '<tr class="ctrtablerow altbg2"><td>'.$lang['whoodump_confirm'].'<br /><form action="tools.php?action=whosonlinedump" method="post"><input type="submit" name="yessubmit" value="'.$lang['textyes'].'" /> - <input type="submit" name="yessubmit" value="'.$lang['textno'].'" /></form></td></tr>';
        } elseif ($lang['textyes'] == $yessubmit) {
            $db->query("DELETE FROM $table_whosonline");

            nav($lang['tools']);
            echo '<tr class="ctrtablerow altbg2"><td>'.$lang['tool_completed'].' '.$lang['tool_whosonline'].'</td></tr></table></td></tr></table>';
            end_time();
            eval('echo "'.template('footer').'";');
            exit;
        } else {
    ...
    case 'fixorphanedthreads':
        if (!isset($_POST['orphsubmit'])) {
            echo '<tr class="tablerow altbg2"><td>';
            ...
            echo '<tr class="ctrtablerow altbg2"><td>'.$i.$lang['o_threads_found'].'</td></tr>';
            nav($lang['tools']);
            echo '<tr class="ctrtablerow altbg2"><td>'.$i.' '.$lang['o_threads_found'].'</td></tr></table></td></tr></table>';
            end_time();
            eval('echo "'.template('footer').'";');
            exit;
        }
    ...
    case 'fixorphanedattachments':
        ...
        */
        if (!isset($_POST['orphattachsubmit'])) {
            echo '<tr class="ctrtablerow altbg2"><td>';
            ...
            nav($lang['tools']);
            echo '<tr class="ctrtablerow altbg2"><td>'.$i.' '.$lang['o_attachments_found'].'</td></tr></table></td></tr></table>';
            end_time();
            eval('echo "'.template('footer').'";');
            exit;
        }
