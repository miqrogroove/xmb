<?php
/* $Id: cp.php,v 1.18 2006/03/04 12:14:36 Jamie Exp $ */
/*
    XMB 1.10
...
if ($action == "settings") {
    if (!isset($_POST['settingsubmit'])) {
        ...
        $bbrulestxtnew = safeAddslashes($bbrulestxtnew);
        $bboffreasonnew = safeAddslashes($bboffreasonnew);
        $tickercontentsnew = safeAddslashes($tickercontentsnew);
    ...
    if (!isset($_POST['forumsubmit']) && !isset($fdetails)) {
        ...
        if (!isset($fdetails)) {
            ...
            if ($newfname != $lang['textnewforum']) {
                $newfname = safeAddslashes($newfname);
            ...
            if ($newgname != $lang['textnewgroup']) {
                $newgname = safeAddslashes($newgname);
            ...
            if ($newsubname != $lang['textnewsubf']) {
                $newsubname = safeAddslashes($newsubname);
            ...
            $namenew = safeAddslashes($namenew);
            $descnew = safeAddslashes($descnew);
        ...
        while ($mem = $db->fetch_array($query)) {
            ...
            if ($delete != "") {
                ...
                if (strpos($pw, '"') !== false || strpos($pw, "'") !== false) {
                    ...
                    $newcustom = safeAddslashes($cusstatus);
...
if ($action == "ipban") {
    if (!isset($_POST['ipbansubmit'])) {
        ...
        while ($ipaddress = $db->fetch_array($query)) {
            ...
            $ipdate = printGmDate($ipaddress['dateline']).' '.$lang['textat'].' '.printGmTime($ipaddress['dateline']);
            ...
            if ($invalid) {
                ...
                if ($ip[1] == '-1' && $ip[2] == '-1' && $ip[3] == '-1' && $ip[4] == '-1') {
                    ...
                    if ($result) {
                        ...
                        $query = $db->query("INSERT INTO $table_banned (ip1, ip2, ip3, ip4, dateline, id) VALUES ('$ip[1]', '$ip[2]', '$ip[3]', '$ip[4]', '$onlinetime', 0)");
                    }
                }
            }
        }
