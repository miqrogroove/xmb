<?php
/* $Id: cinst.php,v 1.8 2006/02/24 18:36:41 Tularis Exp $ */
/*
    XMB 1.10
...
show_act("Checking Super Administrator Account");
    ...
    if ( $vUsername == '' || $frmPassword == '' || $vEmail == '' ) {
        ...
        error('Bad super administrator credentials', 'The username, password or e-mail address cannot be blank or malformed. Please press back and try again.', true);
    }

	if ( $frmPassword != $frmPasswordCfm ) {
		show_result(X_INST_ERR);
		error('Bad super administrator credentials', 'The passwords do not match. Please press back and try again.', true);
	}
...
show_act("Inserting data into ".$tablepre."forums");
    $db->query("INSERT INTO ".$tablepre."forums VALUES ('forum', 1, 'Default Forum', 'on', '', '', 0, '1', 'This is your default forum which is created during installation<br />To add or modify forums goto your control panel - forums', 'no', 'yes', 'yes', '', 0, 0, 0, 0, '1|1', 'yes', 'on', 'on', '', 'off');");
show_result(X_INST_OK);
...
show_act("Inserting data into ".$tablepre."templates");
    ...
    foreach ($templates as $key=>$val) {
        ...
        if ( isset($template[1]) ) {
            $template[1] = safeAddslashes($template[1]);
        } else {
            $template[1] = '';
        }
        $db->query("INSERT INTO ".$tablepre."templates (`name`, `template`) VALUES ('', '".safeAddslashes($template[0])."', '".safeAddslashes($template[1])."')");
    }
