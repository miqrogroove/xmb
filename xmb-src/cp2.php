<?php
/* $Id: cp2.php,v 1.14 2006/08/28 15:26:41 Roxas Exp $ */
/*
    XMB 1.10
...
$auditaction = safeAddslashes("$onlineip|#|$auditaction");
...
elseif ($action == "themes") {
    if (!isset($themesubmit) && !isset($single) && !isset($importsubmit)) {
        ...
        $query = $db->query("SELECT count(themeid) FROM $table_themes WHERE name='".safeAddslashes($name)."'");
...
if (isset($censorsubmit)) {
    ...
    if (!isset($newslettersubmit)) {
        ...
        $newssubject = safeAddslashes($newssubject);
        $newsmessage = safeAddslashes($newsmessage);
        ...
        $_xmbuser = safeAddslashes($xmbuser);

        if ($sendvia == "u2u") {
            while ($memnews = $db->fetch_array($query)) {
                $db->query("INSERT INTO $table_u2u ( u2uid, msgto, msgfrom, type, owner, folder, subject, message, dateline, readstatus, sentstatus ) VALUES ('', '".safeAddslashes($memnews['username'])."', '".$_xmbuser."', 'incoming', '".safeAddslashes($memnews['username'])."', 'Inbox', '$newssubject', '$newsmessage', '" . time() . "', 'no', 'yes')");
            }
        } else {
...
elseif ($action == "templates") {
    ...
    if (isset($editsubmit)) {
        $templatenew = safeAddslashes($templatenew);
        if ($tid == "new") {
            if (empty($namenew)) {
...
elseif ($action == "modlog") {
    ...
    while ($recordinfo = $db->fetch_array($query)) {
        $date = printDmDate($recordinfo['date']);
        $time = printGmTime($recordinfo['date']);
...
elseif ($action == "cplog") {
    ...
    while ($recordinfo = $db->fetch_array($query)) {
        $date = printGmDate($recordinfo['date']);
        $time = printGmTime($recordinfo['date']);
