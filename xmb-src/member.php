<?php
/* $Id: member.php,v 1.3.2.20 2006/03/03 22:24:06 FunForum Exp $ */
/*
    XMB 1.9.2
...
if ($action == "coppa") {
    ...
    if (!isset($regsubmit)) {
        ...
        if ( !file_exists(ROOT.'/lang/'.basename($newlangfile).'.lang.php') ) {
        ...
        $dateformatnew = (intval($dateformatnew) > 0 ? $SETTINGS['dateformat'] : checkInput($dateformatnew, '', '', "javascript", false)); // Temporary validation of dateformat - if it contains numbers we assume the date is incorrect (blacklist approach) and therefore use the default dateformat otherwise we proceed to validation of input.
