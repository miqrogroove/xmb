<?php
/* $Id: member.php,v 1.19 2006/08/28 15:26:04 Roxas Exp $ */
/*
    XMB 1.10
...
if ($action == "coppa") {
    ...
    if (!isset($regsubmit)) {
        ...
        $email = safeAddslashes(trim($email));
        ...
        $dateformatnew  = (intval($dateformatnew) > 0 ? 'dd-mm-yyyy' : checkInput($dateformatnew, '', '', "javascript", false)); // Temporary validation of dateformat - if it contains numbers we assume the date is incorrect (blacklist approach) and therefore use the default dateformat otherwise we proceed to validation of input.
        ...
        $sig        = safeAddslashes($sig);
        $bio        = safeAddslashes($bio);
        $locationnew    = safeAddslashes($locationnew);
        ...
        if ($SETTINGS['notifyonreg'] != "off") {
            if ($SETTINGS['notifyonreg'] == 'u2u') {
                ...
                while ($admin = $db->fetch_array($mailquery)) {
                    $db->query("INSERT INTO $table_u2u (msgto, msgfrom, type, owner, folder, subject, message, dateline, readstatus, sentstatus ) VALUES ('$admin[username]', '".addslashes($bbname)."', 'incoming', '$admin[username]', 'Inbox', '$lang[textnewmember]', '$lang[textnewmember2]', '" . time() . "', 'no', 'yes')");
                }
            } else {
    ...
    if (!$member) {
        ...
        if ($memberinfo['uid'] == '') {
            ...
            if($memberinfo['bday'] === iso8601_date(0,0,0)) {
				...
				$memberinfo['bday'] = printGmDate(mktime(12,0,0,substr($memberinfo['bday'],5,2),substr($memberinfo['bday'],8,2),substr($memberinfo['bday'],0,4)));
            }
            ...
            // Last post
            $query = $db->query("SELECT t.tid, t.subject, p.dateline, p.pid FROM $table_posts p, $table_threads t, $table_forums f WHERE $restrict AND p.fid=f.fid AND p.author='$memberinfo[username]' AND p.tid=t.tid ORDER BY p.dateline DESC LIMIT 1");
