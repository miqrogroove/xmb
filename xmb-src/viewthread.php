<?php
/* $Id: viewthread.php,v 1.16 2006/02/27 12:02:50 Tularis Exp $ */
/*
    XMB 1.10
...
if (!is_null($goto) && 'lastpost' === safeString($goto, false)) {
    ...
    $page = quickpage($posts, $SETTINGS['ppp']);
...
$thislast = explode('|', $thread['lastpost']);
$lastPid = isset($thislast[2]) ? $thislast[2] : 0;
if(is_null($oldtopics = fetchFromRequest('oldtopics', X_COOKIE))) {
    put_cookie('oldtopics', '|'.$lastPid.'|', time()+600, $cookiepath, $cookiedomain, null, X_SET_HEADER);
} elseif(false === strpos($oldtopics, '|'.$lastPid.'|')) {
    $expire = time() + 600;
    $oldtopics .= $lastPid.'|';
...
if ($SETTINGS['smileyinsert'] == 'on' && $SETTINGS['smiliesnum'] > 0) {
    $max = ($SETTINGS['smiliesnum'] > 16) ? 16 : $SETTINGS['smiliesnum'];
